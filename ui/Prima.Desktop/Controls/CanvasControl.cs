...
public sealed class CanvasControl : Control, IDisposable
{
    ...
    {
        get => _document;
        set { _document = value; _fullRenderPending = true; InvalidateVisual(); }
    }
    ...
    private unsafe void RenderIntoBitmap(double scaling, int pw, int ph)
    {
        ...
        else
        {
            ...
            // Map canvas dirty rect → physical target rect with 1px pad for AA.
    ...
    {
        // A stroke dirty rect is pending; mark the control for re-render.
        InvalidateVisual();
    }
